...
void ImageWriter::beginOSListFetch() {
    ...
    // The blank starter catalog is compiled into the application, so load it
    // directly instead of passing the qrc URL to libcurl.
    if (topUrl.scheme() == QLatin1String("qrc")) {
        QFile catalogFile(QLatin1Char(':') + topUrl.path());
        if (!catalogFile.open(QIODevice::ReadOnly)) {
            onOsListFetchError(tr("Unable to open built-in catalog"), topUrl);
            return;
        }
        onOsListFetchComplete(catalogFile.readAll(), topUrl, topUrl);
        return;
    }
